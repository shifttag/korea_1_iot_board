...
<< 설치 명령어 >>
: npm i react-cookie
: npm i --save @types/react-cookie    # 타입 충돌나면 설치하는 명령어
...
export default function D_react_cookie() {
  ...
  /*
    ...
    expires: 쿠키의 만료 시간을 설정
      - 특정 날짜와 시간을 지정하여 만료되도록 설정(특정 시점에 쿠키가 만료됨)
    maxAge: 쿠키의 유효 시간을 초 단위로 설정
      - 현재 시간부터 지정한 시간 동안 쿠키가 유지(특정 시간 동안 쿠키가 유지되고 이후 만료됨)
        (생성된 지점으로부터 시간을 기준으로 유효시간 설정)
